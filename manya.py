...
try:
    ...
    if st.button("Compute Probability"):
        probab2=pd.DataFrame({"Company":nameoc,"Position":nameor,"Probability":finloc})
        st.dataframe(probab2)
        explode_value=(0.08,0.08,0.08,0.08,0.08,0.08,0.08,0.08,0.08,0.08,)
        y = np.array(newx[:10])
        fig=plt.figure()
        fig.patch.set_facecolor('#010f20')
        ...
        # fig.label.set_color('white')
        # plt.text(3, 2, nameor[:10], fontsize=15,color='white')
        st.pyplot(plt)
